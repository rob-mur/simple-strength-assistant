...
- [x] #1 Tapping a set row in the history view opens the edit modal
- [x] #2 Modal is pre-populated with the set's current weight, reps, and RPE
- [x] #3 TapeMeasure is configured with the correct `min_weight` and `increment` for that exercise
- [x] #4 Save persists the edited values; the feed reflects the update without a full reload
- [x] #5 Delete removes the set from the feed; the row disappears immediately
- [x] #6 Deleting the last set in a day/exercise group removes that group from the feed
- [x] #7 E2E: edit weight/reps/RPE and save → updated values appear in feed; delete set → row removed; delete last set in group → group disappears
<!-- AC:END -->
