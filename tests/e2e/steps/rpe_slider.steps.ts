...
When(
  "I change the RPE slider value to {string}",
...
Then(
  "the RPE slider value should be {string}",
...
Then(
  "the RPE slider should have the {string} class",
  async ({ page }, expectedClass: string) => {
    const slider = page.locator('.rpe-slider-container input[type="range"]');
...
When(
  "I press the {string} key on the slider",
...
Then("the RPE slider value should increase", async ({ page }) => {
  const slider = page.locator('.rpe-slider-container input[type="range"]');
  const newValue = await slider.inputValue();
  expect(parseFloat(newValue)).toBeGreaterThan(
    parseFloat(process.env.RPE_INITIAL_VALUE || "0"),
  );
...
Then("the RPE slider value should decrease", async ({ page }) => {
  ...
  expect(parseFloat(finalValue)).toBeLessThan(
    parseFloat(process.env.RPE_LATEST_VALUE || "10"),
  );
});

Then(
  "the RPE slider value should snap to a half-point increment",
  async ({ page }) => {
    const slider = page.locator('.rpe-slider-container input[type="range"]');
...
Then(
  "the RPE slider HTML attributes should be correctly set",
  async ({ page }) => {
    const slider = page.locator('.rpe-slider-container input[type="range"]');
