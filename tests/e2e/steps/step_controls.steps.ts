...
When("I click the increment button", async ({ page }) => {
  const incrementButton = page
    .locator("button.btn-circle.text-success")
    .first();
      ...
      .waitForFunction(
        (initial) => {
          const element = document.querySelector(
            '.tape-measure-container text[text-anchor="middle"]',
          );
...
Then(
  "the step control value should increase or stay at max",
  async ({ page }) => {
    const tape = page.locator(".tape-measure-container").first();
    if (await tape.isVisible()) {
      const newValue = await tape
        .locator('text[text-anchor="middle"]')
        .first()
        .textContent();
      const initialValue = process.env.STEP_INITIAL_VALUE;
      if (initialValue && newValue) {
        expect(parseFloat(newValue)).toBeGreaterThanOrEqual(
...
Then(
  "the step control value should decrease or stay at min",
  async ({ page }) => {
    const tape = page.locator(".tape-measure-container").first();
    if (await tape.isVisible()) {
      const newValue = await tape
      ...
      const initialValue = process.env.STEP_INITIAL_VALUE;
      if (initialValue && newValue) {
        expect(parseFloat(newValue)).toBeLessThanOrEqual(
          parseFloat(initialValue),
        );
      }
    }
...
When(
  "I click the decrement button many times to reach minimum",
  async ({ page }) => {
    const tape = page.locator(".tape-measure-container").first();
    if (await tape.isVisible()) {
      const decrementButton = page
        .locator("button.btn-circle.text-error")
        .last();
      if (await decrementButton.isVisible()) {
        for (let i = 0; i < 20; i++) {
          await decrementButton.click();
...
Then("the step control value should stay at the minimum", async ({ page }) => {
  const tape = page.locator(".tape-measure-container").first();
  if (await tape.isVisible()) {
    const stillMinValue = await tape
      .locator('text[text-anchor="middle"]')
      .first()
      .textContent();
...
Then(
  "there should be multiple increment and decrement buttons",
  async ({ page }) => {
