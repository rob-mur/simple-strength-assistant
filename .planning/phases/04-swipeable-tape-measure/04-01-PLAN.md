...
<task type="auto">
  <name>Task 1: Define BDD Feature for Core Dragging and Scroll Locking</name>
  <files>tests/features/tape_measure_core.feature</files>
  <action>
    - Create a new directory `tests/features/` if it doesn't exist.
    - Write Gherkin scenarios for:
        1. "Smooth dragging" (pointer movement maps to offset change).
        2. "Scroll locking" (verifying `touch-action: none` prevents browser scroll).
        3. "Pointer capture" (dragging continues even if pointer leaves the element).
  </action>
  <verify>
    File `tests/features/tape_measure_core.feature` exists and contains Gherkin scenarios.
  </verify>
  <done>
    BDD scenarios for core interaction defined.
  </done>
</task>

<task type="checkpoint:human-verify" gate="blocking">
  <what-built>BDD Feature Scenarios for TapeMeasure core dragging.</what-built>
  <how-to-verify>
    Review the content of `tests/features/tape_measure_core.feature` and approve the scenarios for dragging and scroll locking.
  </how-to-verify>
  <resume-signal>Type "approved" or describe issues</resume-signal>
</task>

<task type="auto">
  <name>Task 3: Create components module and TapeMeasure scaffold</name>
...
<task type="auto">
  <name>Task 4: Implement Pointer Event Handlers and Capture</name>
