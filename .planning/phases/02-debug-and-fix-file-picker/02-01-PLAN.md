...
must_haves:
  ...
  key_links:
    ...
    - from: "src/app.rs onclick"
      to: "DatabaseManager::initialize"
      via: "inline after prompt_for_file success"
      pattern: "prompt_for_file.*Ok.*DatabaseManager::initialize"
...
Note: ERR-04 (stack trace capture) is scoped to the file picker WASM-JS boundary only in this plan. Other WASM-JS boundaries (db.rs, workout_state.rs) are not audited here.
</objective>
